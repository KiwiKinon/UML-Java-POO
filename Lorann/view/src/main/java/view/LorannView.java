...
public class LorannView implements Runnable, KeyListener, ILorannView {
     ...
     */
    public LorannView(final IMap road, final IMobile myVehicle, final IMobile monster1, final IMobile monster2, final IMobile monster3, final IMobile monster4) throws IOException {
    ...
    private void setRoad(final IMap road) throws IOException {
        ...
        for (int x = 0; x < this.getRoad().getWidth(); x++) {
            for (int y = 0; y < this.getRoad().getHeight(); y++) {
                ...
                if (this.getRoad().getOnTheRoadXY(x, y).getSprite().getConsoleImage()=='o') {
                	System.out.println("Il y a une boule à : ");
              		 BX = x;
              		System.out.println(BX + " x : " + x);
              		 BY = y;
              		System.out.println(BY + " x : " + y);
